...
```{r}
if(run_everything) source("LDA_analysis.R")
```
...
A range of 2-20 topics was considered.
Fit was evaluated based on the BIC, and interpretability was evaluated on the entropy of the posterior probabilities of documents to be assigned to the different topics.
The BICs followed a near-perfect linearly increasing trend, indicating that no subcorpora could be identified.
Congruently, all entropies were near-zero, which means that the posterior probabilities of documents to be assigned to the different topics were uniformly distributed.
Thus, no interpretable subcorpora could be identified, and we proceed with an analysis of the whole sample. <!-- Probably because each set of keywords has only a few indicators. -->
