...
To map the cooccurrence of terms within documents,
it is pertinent to select the most common terms, instead of using IDF.
As a cutoff, we required terms to occur in at least $0.5%$ of documents.
This selection criterion resulted in a corpus of `r dim(dtm_top)[1]` documents and `r dim(dtm_top)[2]` terms.
The marginal occurrence of the selected terms is displayed in Figure \@ref(study1wordcloud).
To determine the frequency with which terms were addressed in documents,
we constructed a term-document matrix

```{r study1wordcloud, fig.cap="Wordcloud of terms selected for Study 1 co-occurrence analysis. More common words are centered and printed in larger font."}
knitr::include_graphics("study1_wordcloud.png")
```
