...
many *relevant* theories are commonly invoked in empirical work.
...
The left panel of Figure \@ref(fig:figbaseline) visually summarizes the phenomena discussed in the theoretical literature,
...
One such domain-specific theory is Hall's notion of "storm and stress" [see
...
In contemporary theory, this notion of diminished self-control and increased emotional reactivity is recast as a normative change that facilitates emotional maturation at the risk of emotional
disturbance [@arnettAdolescentStormStress1999] 
A potential limitation of conceptualizing emotion regulation difficulties as normative aspects of development is that adolescents' diverging destinies are underemphasized [cf. @croneUnderstandingAdolescencePeriod2012].
...
This model posits that individuals use `r keyw("strategies")` to modulate the different stages of this process,
...
Literature reviews can shed a light on undertheorized relevant phenomena by
synthesizing inductive insights emerging from the empirical literature.
This is especially important given the noted absence of a single overarching theory [see @bussTheoriesEmotionalDevelopment2019].
...
as opposed to deductive (confirmatory) research, and as such does not test any hypotheses [ @wagenmakersCreativityVerificationCyclePsychological2018].
...
By identifying theoretically relevant phenomena based on a text-mining systematic review of the literature, the present study lays the groundwork for future theory development.
...
p <- quote({
  ...
  text(-1.1, y = -1, labels = "A")
  legend(x=-1.1, y=1.1, names(cat_cols), pch=legend_shapes, col=cat_cols, pt.bg=cat_cols, pt.cex=1.5, cex=.7, bty="n", ncol=1)
...
The present study used a novel method, the text mining systematic review (TMSR),
...
This approach considers term frequency to be indicative of a phenomenon's relevance.
In line with this assumption, the results of both analyses
reflected some of the phenomena broadly considered to be relevant in theoretical literature and empirical reviews - particularly those pertaining to neurodevelopment and socialization.
...
This result is consistent with the notion that adolescent emotion regulation is implicated in a range of mental health problems [@leeAdolescentMentalHealth2014],
and underlines the importance of research in this area.

The term frequency analyses also identified several novel themes of relevant phenomena that are underrepresented in the theoretical literature and prior systematic reviews.
...
Finally, sexual development emerged as a theme. Aside from the high-ranking construct (biological) sex, this theme was reflected in the terms puberty, sexual diversity, and romantic relationships.
...
This empirically supports the prior (anecdotal) claim that this literature is somewhat fragmented [@riedigerEmotionRegulationAdolescence2014]. 
...
are under-emphasized in narrative reviews and theory - phenomena such as developmental disorders, physical health, external stressors, structural disadvantage, substance use, identity and moral development, and sexual development.
...
This has implications for future research, as the absence of these phenomena from the co-occurrence networks, and the sparsity of connections in the network, suggest potential knowledge gaps.
Future research might study these unembedded phenomena in conjunction with other more well-established constructs,
...
According to formal theory construction methodology, the next step would be to describe and quantify the nature of the relationship of each phenomenon in relation to adolescent emotion regulation.
...
The key disadvantage of the method used is that these text mining techniques
are not able to extract *meaning* from the literature in the way a sentient reader would.
This limitation is best addressed by considering the results as a starting point for inductive reasoning, further deductive research, or a more detailed narrative review on specific sub-topics.
...
Efforts are currently underway to develop unsupervised algorithms capable of distilling implied causal links from scientific abstracts [@anExtractingCausalRelations2019].
...
-->
There has been a call for the integration of the empirical literature on adolescent emotion regulation.
...
The findings reflected several themes commonly featured in theory and narrative reviews,
such associalization and neurocognitive factors,
which suggests that this is a valid method for identifying relevant phenomena.
...
The findings illustrate how text mining systematic
  reviews, a novel approach, may complement narrative reviews. Future
  theoretical work might integrate these undertheorized themes into
  an overarching framework, and empirical research might consider them as
  promising areas for future research, or as potential confounders in research
