...
All analyses are strictly reproducible,
and the code, data, and historical record of this project are available online at [masked]. 
...
However, as no subcorpora could be identified (see [online supplemental material]()), we proceed with an analysis of the whole sample.
...
The total number of unique terms after dictionary encoding was 
We modeled word frequency using a negative binomial distribution,
and retained those exceeding the $97.5^{th}$ percentile.
This cut-off criterion is arbitrary, but necessary to reduce the number of terms considered from 5299.
...
As in Analysis 1, we tested for heterogeneity using latent dirichlet allocation - but no subcorpora were identified ([see online supplement]()).
