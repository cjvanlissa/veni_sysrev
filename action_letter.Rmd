...
Response: We agree with the Reviewer that "The present paper" section was basically a plan of analysis. In the Revision, we have rewritten this section to summarize the study design, link it to the study aims, and state the intended outcomes. 
Note that this is an inductive (exploratory) study, and as such,
it does not test any hypotheses.
We now explicitly clarify this point in "The present paper".
...
Simply put: All decisions are made by us.
There are infinitely many ways to conduct exploratory analysis on a single data set,
and the present paper represents just one possible way.
The "computerized analytical system", as referred to by the Reviewer,
is nothing more than an explicit representation of our subjective decisions in analyzing the data,
represented as code.

The Reviewer asks for reliability checks, also known as a sensitivity analysis or multiverse analysis [@steegenIncreasingTransparencyMultiverse2016].
...
As the present study is inductive (exploratory),
no hypotheses are tested nor inferences are drawn,
and the number of alternative possible analyses is infinite - thus reliability checks are undefined.
...
1) We have documented the methods in detail in the manuscript, and added online supplemental analyses for more complex analyses that we could not describe in sufficient detail.
...
3) The revised Methods section now explicitly mentions that these are inductive (exploratory) analyses, and as such, depend on subjective decisions:
...
we have made all code, data, and the historical record of this project publicly available in an online research repository at [https://github.com/cjvanlissa/veni_sysrev](https://github.com/cjvanlissa/veni_sysrev). 
...
One limitation is that several subjective decisions were made throughout the analysis process.
...
We have attempted to address this limitation by explicitly mentioning important decisions in the manuscript,
and by making all analysis code and data publicly available,
so that others may explore the effect of different analysis strategies.
...
We now mention this explicitly in the paper:
...
Response: We have attempted to address the Reviewer's comment by writing a single Results and Discussion section, instead of separate sections for each analysis. As this change spans several pages, we do not cite the entire revised section here, but refer to the revised manuscript.
...
Response: The Reviewer comes away with the impression that we are "hiding the ball".
We regret that our writing raises these concerns about a lack of transparency,
...
defined as stable and general features of the world [@bogenSavingPhenomena1988].
We propose that systematic reviews are a suitable method for taking this first step,
based on the assumption that phenomena currently considered to be most stable and general will be more frequently represented in the literature.

> The first step in theory construction methodology is to identify relevant phenomena [@borsboomTheoryConstructionMethodologya].
We have argued that text mining systematic reviews are a relatively comprehensive and objective method for identifying such phenomena, because they can evaluate much larger bodies of literature than sentient readers, and are not biased toward pre-existing ideas.s
...
are under-emphasized in narrative reviews and theory.
...
Specifically, theorists might revisit existing relevant theory
and incorporate relevant phenomena as identified in the present analysis.